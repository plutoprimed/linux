				   ...
				   unsigned long parent_rate)
{
	...
	u32 in, reg, m_frac, m_int, status;
	u64 vco, m;
	u8 i, c;
	...
	m = (vco << fpll->fractional_carry_bit) + (in / 2);
	do_div(m, in);
...
static int altera_c5_fpll_parse_dt(struct altera_c5_fpll *fpll)
{
	...
	for_each_available_child_of_node(np, chan_np) {
		...
		ret = of_property_read_u32(chan_np, "adi,rate",
					   &fpll->channels[cnt].rate);
		if (ret < 0) {
			dev_err(fpll->dev, "%s: Unable to parse channel DT property \"adi,rate\"",
				__func__);
			return ret;
		}

		if (fpll->channels[cnt].rate == 0) {
			dev_err(fpll->dev, "%s: Invalid rate %u defined for channel number %u",
				__func__, fpll->channels[cnt].rate,
				fpll->channels[cnt].num);
			return -EINVAL;
		}
